...
- **azurerm**: RG, `azurerm_cognitive_account`/`_deployment`, `azurerm_api_management`(+MI),
  ...
  `azurerm_api_management_redis_cache`(외부 캐시 연결)
- **azapi**: `Microsoft.ApiManagement/service/backends`(circuit breaker·**Pool**) 및
  `Microsoft.Cache/redisEnterprise`(**Azure Managed Redis** + RediSearch, 키 조회) —
  azurerm에서 안정적으로 표현되지 않거나 신규 SKU라 azapi로 직접 관리
